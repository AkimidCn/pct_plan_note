...
class TomogramPlanner(object):
    ...
    def loadTomogram(self, tomo_file):
        with open(self.tomo_dir + tomo_file + '.pickle', 'rb') as handle:
            ...
            # tomogram.shape = (5, n_slice, n_row, n_col)
    ...
    def plan(self, start_pos, end_pos):
        ...
        self.start_idx[1:] = self.pos2idx(start_pos)  # x轴指向右，y轴指向下，地图左上角为(0,0) cv图像的坐标系
    ...
    def pos2idx(self, pos):
        ...
        idx = np.array([idx[1], idx[0]], dtype=np.float32)  # 交换x,y顺序
        return idx