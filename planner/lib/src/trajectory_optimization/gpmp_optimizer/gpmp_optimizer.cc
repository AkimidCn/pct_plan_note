...
bool GPMPOptimizer::GenerateTrajectory(const std::vector<PathPoint>& input_path,
  ...
  Vector6 x0, xN;
  PathPointToNode(path.front(), x0);   // 取出路径的起点，转换为状态向量x0
  PathPointToNode(path.back(), xN);    // 取出路径的终点
  ...
  graph.add(PriorFactor6(gtsam::Symbol('x', 0), x0, sigma_initial));  // 11b 起点约束 参数[要约束的状态变量，先验值，噪声模型（不确定性）]
  factor_idx++;
  for (int i = 1; i < N; ++i) {
    gtsam::Key last_x = gtsam::Symbol('x', i - 1);
    gtsam::Key this_x = gtsam::Symbol('x', i);
    graph.add(GPPriorFactor(last_x, this_x, dt, kQc));  // 连续性判断
    factor_idx++;
    for (int j = 0; j < interpolate_num_; ++j) {
      double height_hint =
  ...
  graph.add(PriorFactor6(gtsam::Symbol('x', N - 1), xN, sigma_goal));  // 终点约束
  factor_idx++;
...
void GPMPOptimizer::PathPointToNode(const PathPoint& path_point, Vector6& x) {
  ...
  x(0, 0) = path_point.x;  // 位置x
  x(1, 0) = std::cos(path_point.heading) * v;   // 速度x分量
  x(2, 0) = 0.0;                              
  x(3, 0) = path_point.y;
